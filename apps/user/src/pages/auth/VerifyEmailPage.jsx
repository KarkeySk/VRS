...
export default function VerifyEmailPage() {
  ...
  useEffect(() => {
    ...
    const completeVerification = async () => {
      ...
      if (providerError) {
        setStatus('error');
        setMessage(providerError.replace(/\+/g, ' '));
        return;
      }
      ...
      try {
        ...
        if (!cancelled) {
          setStatus('success');
          redirectTimer = window.setTimeout(() => navigate('/dashboard', { replace: true }), 1800);
        }
      } catch (err) {
        if (!cancelled) {
          setStatus('error');
    ...
    <div className="auth-shell verify-shell">
      <aside className="auth-visual verify-visual">
        ...
        <div className="auth-visual-content">
          ...
          <p>Verify your email so your bookings, documents, and trip updates stay tied to the right account.</p>
        </div>
      </aside>
